...
export async function runMonthlyRecap(): Promise<MonthlyRecapResult> {
  const now = new Date();
  ...
  for (const p of candidates) {
    ...
    try {
      ...
      if (accounts.length > 0) {
        data.totalBalance = accounts.reduce((acc, a) => acc + Number(a.balance ?? 0), 0);
      ...
      await sendMonthlyRecap(p.email, data, p.practitionerId);
      sent++;
      console.log(`[monthly-recap] sent uid=${emailToLogId(p.email)} (${p.recapFrequency}, ${period.label})`);
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      console.error(`[monthly-recap] error for ${p.practitionerId}:`, msg);
